...
public class EditSession implements Extent, AutoCloseable {
     ...
     *
     * @param pos
     * @param yaw raw angle in radians
     * @return True if modif has been applied
     * @throws MaxChangedBlocksException
     */
    public int makeHadrienTest(BlockVector3 pos, double yaw, Pattern pattern) throws MaxChangedBlocksException {
        int affected = 0;
        int distance = 3;

        if (pos.getBlockY() < world.getMinY())
            return affected;
        if (pos.getBlockY() > world.getMaxY())
            return affected;

        int delta_y = 0;
        int delta_x = (int) Math.floor(distance * Math.sin(Math.toRadians(yaw)));
        int delta_z = (int) Math.floor(distance * Math.cos(Math.toRadians(yaw)));

        BlockType pumpkin_type = Objects.requireNonNull(BlockTypes.CARVED_PUMPKIN);
        BlockState pumpkin_state = pumpkin_type.getDefaultState();
        BaseBlock pumpkin_block = pumpkin_state.toBaseBlock();

        BlockType iron_type = Objects.requireNonNull(BlockTypes.IRON_BLOCK);
        BlockState iron_state = iron_type.getDefaultState();
        BaseBlock iron_block = iron_state.toBaseBlock();


        System.err.print("pos x: " + pos.getBlockX() + ", y: " + pos.getBlockY() + ", z:" + pos.getBlockZ() + "\n");
        System.err.print("pos_block = delta_x: " + delta_x + ", delta_y: " + delta_y + ", delta_z:" + delta_z + "\n");

        setBlock(pos.add(delta_x, delta_y, delta_z), iron_block);
        setBlock(pos.add(delta_x, delta_y + 1, delta_z), iron_block);
        setBlock(pos.add(delta_x + 1, delta_y + 1, delta_z), iron_block);
        setBlock(pos.add(delta_x - 1, delta_y + 1, delta_z), iron_block);
        setBlock(pos.add(delta_x, delta_y + 2, delta_z), pumpkin_block);
